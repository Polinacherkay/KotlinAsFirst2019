...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((key, value) in stockPrices) {
        if (intermediateRes[key] == null) intermediateRes[key] = mutableListOf(value)
        else intermediateRes[key]!!.add(value)
    }
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val c = chars.toMutableList()
    for (i in c.indices) c[i].toLowerCase()
    val letter = c.toSet()
    val w = word.toLowerCase().toSet()
    return (letter.containsAll(w))
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (i in list.indices) {
        if (result[list[i]] == null) result[list[i]] = mutableListOf(1)
        else result[list[i]]!!.add(1)
    }
    for ((key, value) in result) {
        if (value.size > 1) finalResult[key] = value.size
    }
    return finalResult
}
 ...
 *        )
 */
fun propagateHandshakes(friends: Map<String, Set1<String>>): Map<String, Set1<String>> = TODO()
 ...
 *   ) -> emptySet()
 */
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set1<String> = TODO()
